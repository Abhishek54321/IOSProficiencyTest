...
open class ActivityView: UIView {
    ...
    class var sharedInstance: ActivityView {
        struct Singleton {
            static let instance = ActivityView(frame: CGRect(x: 0,y: 0,width: Config().size,height: Config().size))
        }
    ...
    open class func show(_ animated: Bool, isDisplayIndicatorOnTop : Bool = false) {
        self.show(nil, animated: animated, topMargin: 0, isDisplayIndicatorOnTop : true)
    }
    ...
    open class func show(_ title: String?, animated: Bool, topMargin: Int, isDisplayIndicatorOnTop : Bool = false) {
        let currentWindow : UIWindow = UIApplication.shared.keyWindow!
        
        let loader = ActivityView.sharedInstance
        ...
        var indicatorYpos = height/2
        if(isDisplayIndicatorOnTop){
            indicatorYpos = height/2.5
        }
        
        let center : CGPoint = CGPoint(x: width / 2.0, y: indicatorYpos - CGFloat(topMargin))
    ...
    open class func hide(with Animation : Bool = true) {
        let loader = ActivityView.sharedInstance
        loader.stop(with : Animation)
    }
    ...
    fileprivate func stop(with Animation : Bool = true) {
